...
** The case for iframes
  ...
  external source to the vlabs portal which has to be
...
* Iframe Management
  ...
  content division and the event handlers attached to each
...
* Login and Registration
  ...
  and made REST calls to the login/registration API of Open
  edX.  Since this was not the case, the portal has to
...
** Capture the state
   ...
   change.  Instead [[https://developer.mozilla.org/en-US/docs/Web/API/Window/postMessage][postMessage]] mechanism is used to relay
...
** Embedding the javascript to post message
   ...
   portal whose code is black box, the javascript code to
   post messages from these pages has to be inserted.  Open
   edX platform used nginx as it's web server.  To
   accomplish this, nginx's [[http://nginx.org/en/docs/http/ngx_http_addition_module.html][http addition module]] is
   harnessed.  This module allows to add additional html to
