...
** Set Login url to content iframe
   By default on page loads =contentIframe= is sets to load
...
** Validate cookie
   ...
   value. 
   - If the cookie is set to =session_status=loggedin= then
     =Logout= button will appear and it hides the
     =Registration= button.
...
** validate post message
   ...
   =session_status=logged out= based on login/logout events.
