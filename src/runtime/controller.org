...
var expPromise;
var getExp = function() {
  expPromise = expPromise || getJSON(expResPath);
  return expPromise.then(function(data) {
    return data;
  });
}
...
#+BEGIN_SRC js
  var buildLabsDisplayList = function(labs) {
    ...
    var displayList = labs.map(function(lab) {
      ...
      var assets = filterAssetsbyAssetType(lab['assets'], 'image');
...
* filter Assets by AssetType
  This function will filter the assets based on the given =assetType=.
#+name: filter-labs-by-asset-type
#+BEGIN_SRC js
var filterAssetsbyAssetType = function(assets, assetType){
  return assets.filter(function(asset){
    return asset.asset_type.asset_type == assetType;
  });
};
...
var handleEventOnLab = function(obj) {                                                                               
  ...
  if (labUrl.lastIndexOf("Anytime") != -1)                                                                           
      isOpenedXLab = true;                                                                                           
...
var processPostMessage = function(event) {                                                                           
  if (event.origin !== postServer) {                                                                                 
    ...
    if (msg.indexOf("my courses") != -1) {                                                                           
      ...
      displayBtnsForAuthentication();                                                                                
      if (labUrl == undefined || labUrl == "") {                                                                     
        displayAllDisciplines();                                                                                     
      } else if (isOpenedXLab) {                                                                                     
        displayLab(labUrl);                                                                                          
      } else {                                                                                                       
        displayAllDisciplines();                                                                                     
        openInNewTab(labUrl);                                                                                        
...
#+END_SRC
* Get and display exp count from lds
#+name: fill-exp-count
#+BEGIN_SRC js
var fillExpCount = function() {
  getExp()
    .then(displayexpCount)
    .catch(function(err) {
      console.log("Error from display exp count: " + err);
    });
};

#+END_SRC
* Get and display lab count from lds
#+name: fill-lab-count
#+BEGIN_SRC js
var fillLabCount = function() {
  getLabs()
    .then(filterLabsByPhase)
    .then(displaylabCount)
    .catch(function(err) {
      console.log("Error from display lab count: " + err);
    });
};
...
* Display Exp Count
#+name: display-exp-count
#+BEGIN_SRC js
var displayexpCount = function(data) {
  document.getElementById("expDisplay").innerText = data.length.toLocaleString();
};

#+END_SRC
* Display Lab Count
#+name: display-lab-count
#+BEGIN_SRC js
var displaylabCount = function(data) {
  document.getElementById("labDisplay").innerText = data.length.toLocaleString();
};
...
var getUsers = function() {
  usersPromise = usersPromise || getJSON(userAnalyticsPath);
  return usersPromise.then(function(data) {
    return data;
  });
}
#+END_SRC
* Get and display users count from analytics
#+name: fill-users-count
#+BEGIN_SRC js
var fillUsersCount = function() {
  getUsers()
    .then(displayUsersCount)
    .catch(function(err) {
      console.log("Error from display users count: " + err);
    });
};
...
#+BEGIN_SRC js
var displayUsersCount = function(data) {
  document.getElementById("displayUsersCount").innerText = data['registered_users'].toLocaleString();
};
