...
var buildLabsDisplayList = function(labs) {
  ...
  var displayList = labs.map(function(lab) {
    ...
    return '<div class=" col-md-3 " id='+ lab_name + '> <div class="thumbnail" style="cursor:pointer; padding: 0px !important;"><div class="caption" style="min-height: 110px;"><h4 onclick="javascript:handleEventOnLab(this)"  src="' + path +'" id="' + hosted_url + '" style="text-align:center; color:#2c99ce;" >'+ lab_name +'</h4></a></div></div></div>';
  });
...
* Login Support
  Google+ login support.  Display only one button that
  toggles between =Login= and =Logout=.

#+name: gplus-authentication
#+BEGIN_SRC js
var authenticated = function() {
  return true;
};
