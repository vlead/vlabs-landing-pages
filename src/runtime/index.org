...
* Web App
** Index HTML
   This is the index page with a content iframe where all the results will be loaded.
...
<html>
  ...
  <body>
    <div class="container">
      <iframe id="headerIframe" width="100%" height="430em"  src="header.html"
      name="header_iframe" frameborder="0"> </iframe>
      <iframe id="contentIframe" width="100%" height="450em"
              src="content_iframe.html" name="content_iframe" frameborder="0">
      </iframe>
      <iframe id="footerIframe" width="100%" height="450em" src="http://localhost:5001/footer"
      name="footer_iframe" frameborder="0" scrolling="no"></iframe>
    </div>
  </body>
</html>
...
** Content Iframe
   This is the default home page of the virtual labs.
#+BEGIN_SRC html :tangle templates/content_iframe.html :eval no :noweb yes
<html>
  <head>
    <link rel="stylesheet" href="https://maxcdn.bootstrapcdn.com/bootstrap/3.3.7/css/bootstrap.min.css">
    <script src="https://ajax.googleapis.com/ajax/libs/jquery/3.2.0/jquery.min.js"></script>
...
** JS
*** on Load
    on loading of the web page, All the combo boxes will be populated with
...
#+END_SRC
*** Get Labs based on Labname Keyword
    =GetLabsByKeyword= function returns all the labs based on the lab-name
    keyword. This function calls an api 
    =http://<servername>/labs?keyword_lab_name=<keyword>= and displays the
    result in the content iframe.
...
function getLabs(){
  ...
  $.getJSON(lab_url, function(data) {
    ...
    $.each(data, function(key, value){
      lab_url = value['lab_name'][0]['hosted_url'];
     // lab_name = value['lab_name'];
      hosted_url = value['hosted_info'][0]['hosted_url'];
...
#+END_SRC
*** Manage Frames
    =Manage Frames()= function takes the =labs= as input and displays the
    labs in the body =iframe=. When this function is called twice, it
    overrides the previous results.

#+BEGIN_SRC javascript :tangle static/js/main.js :eval no :noweb yes
function manageFrames(labs){
  console.log("==manageFrames==");
  iframe =parent.frames['contentIframe']
  iframe.setAttribute('src', "http://localhost:8000/templates/content_iframe.html");
...
#+END_SRC
*** Sign In 
    =getSignInPage()= function has to load the sign page of virtual labs into
    content iframe.

#+BEGIN_SRC javascript :tangle static/js/main.js :eval no :noweb yes
function getSignInPage(){
  labs = "<html><head><body><div><ul><li><a href='http://vlabs.ac.in'>hello</a></li></ul><p>hello></p></div></body></head></html>"
  manageFrames(labs)
}
#+END_SRC

*** Display All Institutes
    =getAllInstitutes()= function returns all the =Institutes=. This function
    calls an api =http://<servername>/institutes= and appends the result to the
    =Institutes= combo box.
...
function getAllInstitutes(){
  ...
  $.getJSON(inst_url, function(data) {
    ...
    $.each(data, function(key, value){
      ...
      labs += '<li onclick="getLabsByInstitute()" value="' + inst_name +
        '" id="' + inst_name + '"><a href="#">' +
        inst_name + '</a></li>';
    });
    
    $('.filterInstComboBox').append(labs);
  })
    .done(function() {
      console.log( "second success" );
    })
...
*** Display All Disciplines
    =getAllDisciplines()= function returns all the =Disciplines=. This function
    calls an api =http://<servername>/disciplines= and appends the result to the
    =Disciplines= combo box.
...
function getAllDisciplines(){
  ...
  $.getJSON(dis_url, function(data) {
    ...
    $.each(data, function(key, value){
      ...
      labs += '<li onclick="getLabsByDiscipline()" value="' + dis_name +
        '" id="' + dis_name + '"><a href="#">' +
        dis_name + '</a></li>';
    });
    $('.filterDisComboBox').append(labs);
  })
    .done(function() {
      console.log( "second success" );
    })
...
#+END_SRC
*** Display All Labs By Discipline
    =GetLabsByDiscipline()= function returns all the labs based on the
    discipline name selected from =Discipline= Combo box. This function calls
    an api =http://<servername>/labs?discipline_name=<discipline_name>= and
    displays the result in the content iframe.
...
#+END_SRC
*** Display All Labs By Institute
    =GetLabsByInstitute()= function returns all the labs based on the
    institute name selected from =Institute= Combo box. This function calls
    an api =http://<servername>/labs?institute_name=<institute_name>= and
    displays the result in the content iframe.
...
//          + lab_name +'<div class="thumbnail"><img src='+ image +'></img></a></div></div>';
        ...
        labs += '  <span id="getLabsbyInst"></span><div class="col-md-4" id=' + lab_name + '><a href='+ hosted_url + '>'
          + lab_name +'<div class="thumbnail"></a></div></div>';
...
#+END_SRC
*** Display All Labs
    =getAllLabs()= function returns all the labs when clicked on =All Labs=
    tab. This function calls an api =http://<servername>/labs= and displays the
    result in the content iframe.
...
** CSS
   Bootstrap framework default classes are used to style the vlabs landing page.
** Footer
...
#+END_SRC
** Header
   This is the header page of virtual labs. 
  ...
  <nav class="navbar navbar-fixed-top navbar-bootsnipp animate" role="navigation" style="z-index: 9999999">
    ...
    <div class="container-fluid">
      ...
      <ul class="nav navbar-nav">
        ...
        <li><a href="javascript:getAllLabs()" target="content_iframe">All
            Labs</a></li>
    ...
    <div>
      <img src="../static/img/bg1.jpg" style="height:350em position:relative; width:100%;">
    </div>
  </nav>
