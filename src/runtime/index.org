...
var getLabsByDiscipline = function(discId) {
  var labByDiscResPath = lds_url + "/labs?discipline_name=" + discId;
...
var buildDisciplineDisplayList = function(disciplines) {
  ...
  var displayList = disciplines.map(function(discipline) {
    ...
    return '<div class="col-lg-3 col-md-4 col-xs-12 thumb" onclick="postForLabsOfDiscipline(this)" src="' +
      image_path + '" id="' + discipline_id + '"><div class="thumbnail" style="cursor:pointer; padding: 0px !important; padding-top:15px !important;"> <a><img class="img-responsive " onclick="postForLabsOfDiscipline(this)" src="' + image_path +'" id="' + discipline_id + '"><div class="caption" style="min-height: 110px;"><h4 onclick="postForLabsOfDiscipline(this)"  src="' + image_path +'" id="' + discipline_id + '" style="text-align:center; color:#2c99ce;" >'+ discipline_name +'</h4></a></div></div></div>';
  });
...
var buildInstituteDisplayList = function(institutes) {
  ...
  var displayList = institutes.map(function(institute) {
    ...
    return '<div class="col-lg-3 col-md-4 col-xs-12 thumb" onclick="postForLabsOfInstitute(this)" src="' +
      image_path + '" id="' + institute_id + '"><div class="thumbnail" style="cursor:pointer; padding: 0px !important; padding-top:15px !important;"> <a><img class="img-responsive " onclick="postForLabsOfInstitute(this)" src="' + image_path +'" id="' + institute_id + '"><div class="caption" style="min-height: 110px;"><h4 onclick="postForLabsOfInstitute(this)"  src="' + image_path +'" id="' + institute_id + '" style="text-align:center; color:#2c99ce;" >'+ institute_name +'</h4></a></div></div></div>';
  });
...
var buildLabsDisplayList = function(labs) {
  ...
  var displayList = labs.map(function(lab) {
    ...
    return '<div class=" col-md-3 " id='+ lab_name + '> <div class="thumbnail" style="cursor:pointer; padding: 0px !important;"><a><img class="img-responsive" onclick="javascript:postForLabUrl(this)" src="' + path +'" id="' + hosted_url + '"><div class="caption" style="min-height: 110px;"><h4 onclick="javascript:postForLabUrl(this)"  src="' + path +'" id="' + hosted_url + '" style="text-align:center; color:#2c99ce;" >'+ lab_name +'</h4></a></div></div></div>';
  });
...
** Attach the display content to an Iframe
   ...
   3. This iframe isp inserted into the =ContentDiv= of the
      index page. 
...
var processPostMessage = function(event) {
  if (event.origin !== postServer) {
    ...
    if (!recvMsg.edx) {
      if (recvMsg.action == "display-labs-of-discipline")
        displayLabsOfDiscipline(recvMsg.id);
      else if (recvMsg.action == "display-labs-of-institute")
        displayLabsOfInstitute(recvMsg.id);
      else if (recvMsg.action == "display-lab")
        displayLab(recvMsg.id);
    }
  }
};
