...
** Create Div content for all Institutes
...
*** TODO [0/1]
    1. [ ] Pass a key to identify the html file for the
       institutes.
*** Realization
#+NAME: create-institutes-div-content
#+BEGIN_SRC javascript 
var createInstitutesDivContent = function(){ 
...
<h4 class="cursor-pointer" style="text-align:center; color:#2c99ce;" >'+ institute_name +'</h4></div></div></div>';
...
var setSourceToIframe = function(htmlFile) {
  ...
  cIframe.src = HTML_FILES_PATH + "/" + htmlFile;
};
...
** Set Content For Iframe
   ...
   =commonDiv= that is common in all the different html
   files.
...
var setContentToIframe = function(content) {
  ...
  var elem = contentDoc.getElementById("commonDiv");
...
#+NAME: create-footer
#+BEGIN_SRC js
var createFooterFrame = function() {
