...
var attachDisciplineDisplayListToIframe = function(content) {
  ...
  setContentInContentDiv(content);
};
...
#+END_SRC
** Display the in a sandboxed iframe
...
* Set Content in ContentDiv
  HTML content is set to the contentDiv of the main page.
  Before content is set, the older content is deleted. 
