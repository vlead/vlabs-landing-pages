...
#+END_SRC
** Get All Labs
      We show this by loading a =lab_url= as source of the iframe.
#+NAME: get-all-labs
#+BEGIN_SRC  javascript 
...
** Get Labs by Discipline Name
      We show this by loading a =labs_by_dis_url= as source of the iframe.
...
** Get Labs by Institute Name
 We show this by loading a =labs_by_institute_url= as source of the iframe.
...
** Add Script Tags 
   Resetting the content in the same frame is by accessing required javascript
   files accessed and the new content is set
