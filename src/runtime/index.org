...
<html lang="en">
  <head>
    ...
    <!--<script type="text/javascript" src="../static/js/backfix.min.js"></script>-->
    <script>
...
var setLoginUrlInContentIframe = function(event) {
  console.log("=======setting login url to content iframe=====");
    ...
    if(event == "login"){
        ...
        parent.window.loggedin = false;
        parent.document.cookie = "loginHandler='logged out'";
        document.getElementById("login").innerHTML = "Login";
        document.getElementById("login").setAttribute( "onClick", "setLoginUrlInContentIframe('login')" );
        loadAllDisciplines();
  }
...
var validateAuthentication = function(url) {
    var lab_url = url.id;
    var loggedinHandler = parent.window.loggedin;
    var iframe = parent.document.getElementById("contentIframe");
    if(loggedinHandler == false){
        parent.window.url = lab_url;
        iframe.src = OPENEDX_URL + "/login";
    }
    else{
        iframe.src = lab_url;
...
var setNewContentInIframe = function(data) {
  ...
  var iframe = parent.document.getElementById("contentIframe");
  iframe.sandbox = 'allow-forms allow-scripts allow-same-origin allow-popups allow-top-navigation';
...
institutes += '<div class="col-lg-3 col-md-4 col-xs-6 thumb" style="cursor: \
...
var getAllDisciplines = function(){ 
  ...
  $.getJSON(dis_url, function(data) {
    ...
    var image_path = discipline_images_url + image_name;
     disciplines += '<div class="col-lg-3 col-md-4 col-xs-6 thumb" style="cursor: \
...
var getLabsByDisciplineName = function(image, flag) {
  ...
  $.getJSON(labs_by_dis_url, function(data) {
    ...
    $.each(data, function(key, value){
      ...
      labs += '<div class=" col-md-3 " style="cursor:pointer;" id='+ lab_name +
           '> <div class="thumbnail" style="padding: 0px !important;"><a id="' + hosted_url + '" onclick="javascript:validateAuthentication(this);"><img class="img-responsive" src="' + path +
           '"alt="image"/><a href='+ hosted_url +
           '> <div class="caption" style="min-height: 110px;" ><h4 style="text-align:left; color:#2c99ce;">'+lab_name +
           '</h4></div></a></div></div>';
      });
...
//        '</h4></div></a></div></div>';
      labs += '<div class=" col-md-3 " style="cursor:pointer;" id='+ lab_name +
           '> <div class="thumbnail" style="padding: 0px !important;"><a id="' + hosted_url + '" onclick="getLab(this);"><img class="img-responsive" src="' + path +
           '"alt="image"/><a href='+ hosted_url +
           '> <div class="caption" style="min-height: 110px;" ><h4 style="text-align:left; color:#2c99ce;">'+lab_name +
           '</h4></div></a></div></div>';
...
var addScriptTags = function(){
  ...
  /*var backfix_script = contentIframe.contentWindow.document.createElement("script");
  var backfix_url = "../static/js/backfix.min.js";
  backfix_script.type = "text/javascript";
  backfix_script.src = backfix_url;
  head.appendChild(backfix_script);*/
