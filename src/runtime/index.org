...
var HTML_FILES_PATH = "../templates"
var FOOTER_PATH = "http://footer.vlabs.ac.in/footer";
#+END_SRC
...
** Create Div content for all Disciplines
...
*** TODO [1/2]
    1. [X] Refactor the promise, when should the discipline
       list be returned, should it be =always=.  
       Solution :: There will no return from a promise,
       instead, there will be a callback to insert this
       content in an iframe.
    2. [ ] Refactor the AJAX call so that this can be used
       by all other AJAX calls.

*** Realization
#+NAME: create-disciplines-div-content
#+BEGIN_SRC javascript 
var createDisciplinesDivContent = function(){ 
  console.log("=========inside createDisciplinesDivContent ==========");
  ...
  $.getJSON(dis_url, function(data) {
    ...
    $.each(data, function(key, value){
      ...
      disciplines += '<div class="col-lg-3 col-md-4 col-xs-6 thumb"> <div class="thumbnail"  \
style="padding: 0px !important; padding-top:15px !important;"> <a><img class="img-responsive"\
...
<h4 onclick="getLabsByDisciplineName(this, 0)" src="'
        + image_path +'" id="' + discipline_name + '" style="text-align:center; color:#2c99ce;">'
        + discipline_name +'</h4></div></a></div></div>';
    });
    ...
    setDisciplineContent(discipline_list);
  })
    .done(function() {
...
#+END_SRC
** Set Discipline content in frame
#+NAME: discipline-content
#+BEGIN_SRC js
var setDisciplineContent = function(content) {
  var cIframe = createIframeForContent();
  insertContentIframe(cIframe);
  setDisciplineSourceToIframe();
  setTimeout(setDisciplineContentForIframe, 100, content);
};
...
** Set Discipline Source To Iframe
   The iframe source is set to disciplines html. 
#+NAME: set-discipline-source-to-iframe
#+BEGIN_SRC javascript 
var setDisciplineSourceToIframe = function() {
  var cIframe = document.getElementById("contentIframe");
  cIframe.src = HTML_FILES_PATH + "/disciplines.html";
};

#+END_SRC

** Set Discipline Content 
   The content consisting of different disciplines is set to
   the content iframe.  At this point, the source of the
   content iframe is set to =disciplines.html=.  Therefore
   the content is plugged into the hole marked by
   =discipline-div=.
#+NAME: set-discipline-content-for-iframe
#+BEGIN_SRC javascript 
var setDisciplineContentForIframe = function(content) {
  setContentForIframe(content, "disciplineDiv");
  addScriptTags();
};
...
** Set Content For Iframe
   This is a generic function, where content is set to given
   div id in content iframe.
#+NAME: set-content-for-iframe
#+BEGIN_SRC javascript 
var setContentForIframe = function(content, divId) {
  console.log("divId = " + divId);
  var cIframe = document.getElementById("contentIframe");
  var contentDoc = cIframe.contentDocument;
  var elem = contentDoc.getElementById(divId);
...
** Set Footer Source To Iframe
   Footer is served from a different service.  The source of
   the footer iframe is set to this service.
#+NAME: set-footer-source-to-iframe
#+BEGIN_SRC javascript 
var setFooterSourceToIframe = function(fIframe) {
  fIframe.src = FOOTER_PATH;
  return fIframe;
};
#+END_SRC
...
var insertFooterIframe = function(fIframe) {
  insertFrameInDivision(fIframe, "footerDiv");
};
...
<<load-index>>
<<create-disciplines-div-content>>
<<create-new-iframe>>
<<create-iframe-for-content>>
<<create-iframe-for-footer>>
<<discipline-content>>
<<set-discipline-source-to-iframe>>
<<set-discipline-content-for-iframe>>
<<set-content-for-iframe>>
<<set-footer-source-to-iframe>>
<<insert-content-iframe>>
<<insert-footer-iframe>>
